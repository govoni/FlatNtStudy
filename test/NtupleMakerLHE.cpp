...
public:
  ...
  void fillTree (string fileNameLHE);
...
void myTree::fillTree(string fileNameLHE){
  
  ifstream ifs (fileNameLHE.c_str ());
  LHEF::Reader reader (ifs) ;
 
  long ieve = 0 ;
  cout << "reading " << fileNameLHE << endl;
 
  // loop over events
  while ( reader.readEvent () ) {
    ieve++;
    if (ieve % 10000 == 0) cout << "event " << ieve << "\n" ;
  
    //---- initialize variables in tree
    Init();
  
    vector<myParticle> v_tlv_all_leptons ; //leptons
    vector<myParticle> v_tlv_all_quarks ; //quarks and gluons
    vector<myParticle> v_tlv_all_neutrinos ; //neutrinos
    vector<myParticle> v_tlv_all_vbosons ; //vector bosons
...
int main (int argc, char **argv) {
  ...
  cout << "Input LHE = " << argv[1] << endl;
  cout << "Output ROOT = " << argv[2] << endl;
 
  myTree myT;
  myT.fillTree (string(argv[1])) ;
