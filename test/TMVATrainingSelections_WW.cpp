...
int main (int argc, char** argv){
  ...
  vector<shared_ptr<TMVATrainingClass<cutContainerWW> > > trainingVector ;
  ...
  for(size_t puBin = 0; puBin+1 < pileUpBin.size() ; puBin++){
    ...
    for(size_t iCut = 0 ; iCut < CutList.size(); iCut++){
      ...
      trainingVector.push_back(shared_ptr<TMVATrainingClass<cutContainerWW> > (new TMVATrainingClass<cutContainerWW>(signalMap, 
														     backgroundMap, 
														     treeName, 
														     outputFileDirectory, 
														     outputFileName,
