...
if __name__ == '__main__':
    ...
    for datacard in datacardList :
                ...
                if ifile.split(" ")[0] == "process" and iproc ==0:
                    ...
                    for columns in ifile.split() :
                        if options.datacardType == 0 :
                            if not ROOT.TString(columns).Contains("WW_EWK") :
                                icol = icol+1;
                                continue;
                            else :
                            ...
                            if  not ROOT.TString(columns).Contains("_LL") and not ROOT.TString(columns).Contains("_TL") and not ROOT.TString(columns).Contains("_TT") and not ROOT.TString(columns).Contains("_LT") :
                                icol = icol+1;
                                continue;
                            else :
                                fakeColumn.append(icol);
                            ...
                            if not ROOT.TString(columns).Contains("HminusNoH") and not ROOT.TString(columns).Contains("WW_EWK") :
                                icol = icol+1;
                                continue;
                            else :
                                fakeColumn.append(icol);
            ...
            for key in inputrootfile.GetListOfKeys() :
                    ...
                    if options.datacardType == 0 :
                        if ROOT.TString(key.GetName()).Contains("WW_EWK") :       
                            histo.Scale(options.rescaleFactor);
                    elif options.datacardType == 1:
                        if ROOT.TString(key.GetName()).Contains("_LL") or ROOT.TString(key.GetName()).Contains("_TL") or ROOT.TString(key.GetName()).Contains("_LT") or ROOT.TString(key.GetName()).Contains("_TT")  :
                            histo.Scale(options.rescaleFactor);
                    elif options.datacardType == 2 :
                        if ROOT.TString(key.GetName()).Contains("HminusNoH") or ROOT.TString(key.GetName()).Contains("WW_EWK") :
                            histo.Scale(options.rescaleFactor);
