...
if __name__ == '__main__':
    ...
    for card in totalCards :
        ...
        if options.makeMaxLielihoodFit :
            runCmmd =  "combine -M MaxLikelihoodFit --minimizerAlgo Minuit2 --minimizerStrategy 1 --rMin %d --rMax %d --saveNormalizations --saveWithUncertainties  -n %s -m 100 -d  %s -v 2 --robustFit=1 --do95=1 -s -1 -t %d --expectSignal %d"%(options.rMin,options.rMax,outname,card,options.nToys,options.injectSignal);
            os.system(runCmmd);
            os.system("mv higgsCombine* "+options.outputDIR);
