...
class leptonContainer {
 ...
 public:
  ...
  leptonContainer(TLorentzVector lepton4V, float charge, int flavour, float iso):
   lepton4V_(lepton4V),
   charge_(charge),
   flavour_(flavour),
   iso_(iso){};  
...
class jetContainer {
 ...
 public:
  ...
  jetContainer(TLorentzVector jet4V, float jetPUID, float btag):
   jet4V_(jet4V),
   jetPUID_(jetPUID),
   btag_(btag){};  
...
bool closeToLeptons (float eta, float phi, vector<leptonContainer> & TL_leptons, float R = 0.3) ;
...
void fillRecoLeptonsArray (vector<leptonContainer> & jetVector, readTree & reader) ;
...
vector<leptonContainer> dumpLeptons(std::vector<leptonContainer> & TL_leptons, float isocut, float minptcut) ;
...
vector<jetContainer> dumpJets (vector<jetContainer> & TL_jets, std::vector<leptonContainer>  & TL_leptons, 
