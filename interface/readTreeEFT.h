...
// Tue Mar 17 19:17:49 2015 by ROOT version 5.34/07
// from TTree readTreeEFT/readTreeEFT
// found on file: outDumper_0.root
//////////////////////////////////////////////////////////

#ifndef readTreeEFT_h
#define readTreeEFT_h
...
const Int_t kMaxTrackJet_V4 = 46;
...
#ifdef readTreeEFT_cxx
readTreeEFT::readTreeEFT(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("outDumper_0.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("outDumper_0.root");
      }
      f->GetObject("readTreeEFT",tree);

   }
   Init(tree);
}

readTreeEFT::~readTreeEFT()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t readTreeEFT::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t readTreeEFT::LoadTree(Long64_t entry)
{
...
void readTreeEFT::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("eventLHEweight", &eventLHEweight, &b_eventLHEweight);
...
void readTreeEFT::Show(Long64_t entry)
{
...
Int_t readTreeEFT::Cut(Long64_t entry)
{
